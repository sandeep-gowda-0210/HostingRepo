  ...
  return (!selectedUser || !socket.id ? <div className='flex justify-center items-center h-full border-2 rounded-2xl'>No chat Selected</div> : <div className="flex flex-col h-full border rounded shadow w-full">
    ...
    <div className="flex items-center justify-between p-4 border-b bg-blend-darken">
      ...
      <div className='flex  items-center gap-x-6 '>
        ...
        {!scheduleMessagePage && user?.user_id !== selectedUser.user_id &&
          <svg width="30" height="30" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg" onClick={() => {
            autoReplyPage ? setAutoReplyPage(false) : null; setScheduleMessagePage(true)
    ...
    {/* Messages */}
    {selectedFile ? (
      <div className="h-full w-full flex flex-col items-center justify-center border rounded p-4">
        <div className="text-center">
          <p className="mb-2 font-semibold text-3xl">Input File:</p>
        ...
        <button
          onClick={handleSend} // <-- Make sure you define this handler
          className="mt-6 px-6 py-2 text-xl bg-blue-600 text-white rounded hover:bg-blue-700 transition duration-200"
        >
          Send
        </button>
      </div>
        ...
        <div ref={containerRef} className="flex-1 p-4 py-3 overflow-y-auto space-y-2 bg-blend-darken">
                  ...
                  <div className="w-[300px] max-h-[200px] overflow-hidden rounded bg-black flex justify-center items-center flex-col">
                    <div className='w-[300px] max-h-[150px] h-auto overflow-hidden'>
                      {msg.type.startsWith("image/") ? (
                        ...
                        <iframe src={msg.file_url} className="w-full h-60 rounded" title="PDF Preview" />
                      ) : msg.type.startsWith("video/") ? (
                        <video src={msg.file_url} controls className="max-w-full max-h-60 rounded" />
                      ) : null}
                    </div>
                    <div className='mt-2 bg-[#5b5a5a] p-1 rounded-lg cursor-pointer px-2'>{msg.file_url && (
                      <a
                        href={msg.file_url}
                        download={msg.file?.name}
                        target="_blank"
                        rel="noopener noreferrer"
                        className=" text-blue-300 hover:text-blue-600 break-all"
                      >
          ...
          {!isAtBottom && (
            <button
              ...
              className="fixed text-lg bottom-24 right-10 cursor-pointer bg-blue-600 text-white px-4 py-2 rounded-full shadow-lg hover:bg-blue-700 transition-all z-50"
            >
              v
            </button>
          )}
